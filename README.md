...
**Important**: If you do not use the deafult `MAGENTO_URL` and decides to add an alias on your `/etc/hosts` mapped to your docker machine ip you must use a hostname that contains a dot (e.g `foo.bar`), otherwise the [Magento admin panel login won't work](http://magento.stackexchange.com/a/7773).
